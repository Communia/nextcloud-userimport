...
```
usage: nextcloud_user_importer.py [-h] [--protocol PROTOCOL] --nc-url NC_URL --admin-name ADMIN_NAME --admin-pass ADMIN_PASS [--api-url API_URL]
                                  (--users-csv-file USERS_CSV_FILE | --groups-csv-file GROUPS_CSV_FILE)
                                  [--csv-delimiter CSV_DELIMITER] [--csv-delimiter-groups CSV_DELIMITER_GROUPS]
                                  [--generate-password] [--no-ssl-verify] [--language LANGUAGE] [--dry-run]
